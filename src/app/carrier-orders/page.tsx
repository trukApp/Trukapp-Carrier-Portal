...
const CarrierOrdersPage: React.FC = () => {
  ...
  const { data: assignments, isLoading: isFetching } =
    useGetCarrierAssignmentsQuery(carrierIdFromRedux);
  const getAllCarrierAssignments: CarrierAssignment[] = assignments?.data || [];
  console.log("assignments: ", assignments);
  const handleNavigateToOrder = (orderId: string) => {
    setLoading(true);
    router.push(
      `/detailed-carrier-overview?order_ID=${orderId}&from=${carrierId}`,
    );
  };
  ...
  const rows = getAllCarrierAssignments.map((item, idx) => ({
    ...
    assigned_time:
      moment(item.assigned_time).format("DD MMM YYYY, HH:mm") ?? "—",
...
// import BidFilters from "@/Components/CarrierOrdersComponents/BidFilters";
// import BidTabs from "@/Components/CarrierOrdersComponents/BidTabs";
// import BidTable from "@/Components/CarrierOrdersComponents/BidTable";
