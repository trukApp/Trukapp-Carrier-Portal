...
const Allocations: React.FC<AllocationsProps> = ({
  allocations,
  orderId,
  ...
  const getMaximumDateAndTimeToRequest =
    allocatedPackageDetails[0]?.pickup_date_time;
  console.log(
    "getMaximumDateAndTimeToRequest: ",
    getMaximumDateAndTimeToRequest,
  );
                ...
                >
                  {({ values, setFieldValue, errors, touched }) => (
                    <Form>
                      <DialogContent>
                        ...
                        >
                          <DateTimePicker
                            ...
                            maxDateTime={
                              new Date(getMaximumDateAndTimeToRequest)
                            }
                            ...
                            slotProps={{
                              textField: {
                                ...
                                size: "small",
                                error:
                                  touched.assignmentDateTime &&
                                  Boolean(errors.assignmentDateTime),
                                helperText:
                                  touched.assignmentDateTime &&
                                  typeof errors.assignmentDateTime === "string"
                      ...
                      <DialogActions>
                        <Button onClick={() => setOpenDockRequest(false)}>
                          Cancel
                        </Button>
                        <Button variant="contained" type="submit">
                          Submit
            ...
            <Paper key={uniqueKey} sx={{ p: 2, mb: 2 }}>
              ...
              <Collapse in={expanded[uniqueKey]} timeout="auto" unmountOnExit>
                ...
                >
                  ...
                  {allocatedPackageDetails
                    .filter((pkg: PackageDetails) =>
                      allocation.packages.includes(pkg.pack_ID),
                    )
                    .map((pkg: PackageDetails) => (
                      <Box key={pkg.pac_id}>
                        <Grid
                          key={pkg.pac_id}
                          sx={{
                  ...
                  ) : assignmentData?.dock_allocation_status ===
                    "allocated" ? null : assignmentData?.dock_allocation_status ===
                    "requested" ? (
                    <Typography
                      color="#00000"
                      style={{
